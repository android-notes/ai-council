...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const route = resolveProviderRoute(requestUrl);
    if (!route) {
      return jsonResponse(
        {
          error:
...
function resolveProviderRoute(requestUrl: URL) {
  const [, provider, ...rest] = requestUrl.pathname.split("/");
...
function validateRelayToken(request: Request, env: Env) {
